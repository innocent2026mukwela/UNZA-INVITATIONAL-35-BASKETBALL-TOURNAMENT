const tiers = [
  {
    tier: 'Platinum',
    color: '#e8c97a',
    glow: 'rgba(232,201,122,0.25)',
    border: 'rgba(232,201,122,0.3)',
    bg: 'rgba(232,201,122,0.05)',
    desc: 'Premier visibility across all event touchpoints',
  },
  {
    tier: 'Gold',
    color: '#e8000d',
    glow: 'rgba(232,0,13,0.25)',
    border: 'rgba(232,0,13,0.35)',
    bg: 'rgba(232,0,13,0.05)',
    desc: 'Strong brand presence throughout the tournament',
  },
  {
    tier: 'Silver',
    ...
    desc: 'Quality exposure to our engaged audience',
  },
];
    ...
    <section id="sponsors" className="pt-28 pb-16 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-5 justify-center items-stretch mb-16 reveal">
          {tiers.map((s, i) => (
            <div key={s.tier}
              className="card-lift group flex-1 max-w-sm rounded-2xl p-8 flex flex-col items-center text-center cursor-pointer"
            ...
            >
              ...
              <div className="w-28 h-28 rounded-2xl flex items-center justify-center mb-5 transition-transform duration-300 group-hover:scale-105"
                ...
                <svg className="w-12 h-12 transition-all duration-300" style={{ color: s.color, opacity: 0.5 }}
              ...
              <p className="font-['Inter'] text-xs text-white/45 leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>
